$(document).ready(function() {
	...
	var maxLabel = $('#maxPName').val(), maxData = $('#maxPCount').val(), minLabel = $('#minPName').val(), minData = $('#minPCount').val(), restData = $('#totalSoldProducts').val();
	var dataForm = dataFormatter(maxLabel, maxData, minLabel, minData, restData);
	...
	function dataFormatter(maxLabel, maxData, minLabel, minData, restData) {
		...
		for (var i = 0; i < size; i++) {
			...
			if (i == 2) {
				if (restData != '') {
					var restD = parseInt(restData) - (parseInt(maxData) + parseInt(minData));
					data[i] = {
						label : "Other",
						data : restD
					};
