$(document).ready(function() {
	...
	$('#expiry').datepicker({
		...
		onSelect : function(selected) {
			var dt = new Date(selected);
			dt.setDate(dt.getDate() - 1);
			$("#manufacture").datepicker("option", "maxDate", dt);
		}
	});
	...
	$("#status, #manufacture, #expiry").bind('keyup change', function() {
	...
	$('#createProductBatchTable tbody').on('click', 'tr', function() {
	...
	$('#submit').click(function() {
		...
		// dataTable() instead
		// of DataTable().
		var rowCount = $('#createProductBatchTable').dataTable().fnGetData().length;
		var quant = $('#quantityRequired').val();

		if (rowCount == quant) {
			var tableData = $('#createProductBatchTable').tableToJSON({
				headings : [ 'manufacture', 'expiry', 'status' ]
			});
			// console.log(data);
			...
			if (diff > 0) {
				alert("Add " + absDiff + " more rows, row count should be exect " + quant);
			} else if (diff < 0) {
				alert("Remove " + absDiff + " rows, row count should be exect " + quant);
			}
		}
	});
